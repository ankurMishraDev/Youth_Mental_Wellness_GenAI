...
/**
 * Check if a string is likely encrypted (valid base64 with correct length)
 */
function isEncryptedFormat(value: string): boolean {
  ...
  // IV is 12 bytes, minimum encrypted data ~16 bytes = 28 bytes = ~38 base64 chars
  if (trimmed.length < 20) return false;

  try {
    // Try to decode - if it fails, not valid base64
    const decoded = atob(trimmed);
    // Should have at least IV_LENGTH bytes
 ...
 * IMPORTANT: Also handles plain-text (non-encrypted) data for backward compatibility
 */
export async function decryptField(encryptedValue: string, userId: string): Promise<string> {
  // Handle non-string inputs (already decrypted or invalid data)
  ...
  // Check if data is actually encrypted - if not, return as plain text
  if (!isEncryptedFormat(trimmed)) {
    // This is plain text (old data before encryption was added)
    return trimmed;
  }
  ...
  try {
    ...
    if (combined.length <= IV_LENGTH) {
      console.warn('Encrypted data too short, returning as plain text');
      return trimmed;
    }
    ...
    // If decryption fails, assume it's corrupted or plain text
 ...
 * IMPORTANT: Also handles plain objects (non-encrypted) for backward compatibility
 */
export async function decryptObject<T>(encryptedValue: any, userId: string): Promise<T | null> {
  // If already an object (not encrypted), return it as-is
  if (typeof encryptedValue === 'object' && encryptedValue !== null) {
    return encryptedValue as T;
  }

  // If not a string, can't decrypt
  ...
  if (!isEncryptedFormat(encryptedValue)) {
    // Try to parse as JSON (might be plain JSON string from old data)
    try {
      return JSON.parse(encryptedValue) as T;
    } catch {
      // Not valid JSON either, return null
      return null;
    }
  }
